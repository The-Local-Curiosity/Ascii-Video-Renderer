...
# Order generated by perfect order 
CHARS = '''@WQB$gM0RN8&D#%EOG96mdpqH5bKwPAkUZ4hSy2ea3]XV[CIFo7ujfsn}t{J1i)z(rYTxclLv|=*?><+~!^;-_,':`.  '''
...
def frame_to_ascii(frame_bgr, out_w=OUT_W, out_h=OUT_H):
    ...
    for row in resized:
        line_parts = []
        for (b, g, r) in row:
            # Use perceived brightness (luma) to choose the character
            I = int(0.2126 * r + 0.7152 * g + 0.0722 * b)
            # Brightest (255) -> index 0, Dimmest (0) -> index n-1
            bucket = (255 - I) * n // 256
            ch = CHARS[bucket]
            # Using Single Character for inferencing 
            # ch = 'A'

            # Color the chosen character using the pixel's RGB
            line_parts.append(rgb_text(int(r), int(g), int(b), ch))
